...
export const create = async  (user): Promise<Database["public"]["Tables"]["user"]["Row"]> => { 
    ...
    const isExists = await db.from("user").select().or(`email.eq.${user.email},phone.eq.${user.phone},license_number.eq.${user.license_number}`).single()

    if(isExists.data) throw new Error(ERR_USER_ALREADY_REGISTERED)
