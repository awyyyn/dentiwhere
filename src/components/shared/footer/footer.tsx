        ...
        <div className='flex flex-col gap-y-5 sm:gap-y-none justify-between h-full   '> 
          <div className='space-y-3'> 
            ...
            <div className='space-y-3  pl-5'>
              {generalLinks.map(({name, path}, indx) => (
                <Link to={path} key={`general-link-${path}-${indx}`} className='block hover:drop-shadow-lg transition-all '>
                  {name}
                </Link> 
              ))}
            </div>
          ...
          <div className='space-y-3'>
              ...
              <div className='flex flex-row gap-x-5   pl-5'>
                <Button size="icon" variant="ghost" className='hover:shadow-xl transition-shadow duration-300'>
