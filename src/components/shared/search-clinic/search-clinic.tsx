...
export default function SearchClinic() {
	...
	const Card = (clinic: ClinicWithDoctor) => {
		return <div onClick={() => navigate(`/clinics/view/${clinic.id}`)} className="rounded-lg gap-2 items-center border flex shadow-md hover:shadow-lg transition-all duration-300 cursor-pointer p-2 md:p-5">
			<div>
				<AsyncImage
					alt={`clinic profile`}
					src={clinic.img ?? "https://developers.elementor.com/docs/assets/img/elementor-placeholder-image.png"}
					Transition={props => <Blur radius={20} {...props}/>}
					className="object-fill shadow-sm border rounded-2xl w-20 h-20"
				/>
			</div>
			<div className="space-y-1">
				<h1 className="font-bold text-xl md:text-2xl capitalize">{clinic.name}</h1>
				<div className="flex items-center gap-1">
					<User className="h-4 w-4 md:h-5 md:w-5" />
					<h1 className="capitalize font-semibold md:text-md text-sm text-gray-600">{clinic.doctor}</h1>
				</div>
				<div className="flex items-center gap-1">
					<Contact className="h-4 w-4 md:h-5 md:w-5" />
					<p className="capitalize text-gray-500 md:text-md text-sm">+63{clinic.contacts[0].slice(1)}</p>
				</div>
				<div className="flex items-center gap-1">
					<Map className="h-4 w-4 md:h-5 md:w-5" />
					<p className="capitalize text-gray-500 md:text-md text-sm">{clinic.address}</p>
				</div>
				{/*<p>Rating</p>*/}
			</div>
		</div>
	...
	const SearchLoading = () => <div className="w-full h-96 flex justify-center items-center"><ImSpinner2 size={50} className="animate-spin" /></div>
			...
			<DialogContent
				...
				<div className="max-h-[70vh] md:max-h-[60vh] overflow-y-scroll scrollbar-hide">
					<div className="space-y-4 p-2 pb-10">
						{loading ? <SearchLoading /> : searching ?
							results.length > 1 ? results.map(clinic => <Card key={clinic.id} {...clinic} />)
								: <h1>No Results</h1> : defaultClinics.map(clinic => <Card key={clinic.id} {...clinic} />)}
					</div>
				</div>
			</DialogContent>
		</Dialog>
